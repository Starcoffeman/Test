...
function t898_init(recId) {
  ...
  if (whatsApp) {
    ...
    if (
      whatsAppHref &&
      (whatsAppHref.indexOf("whatsapp://") > -1 || whatsAppHref.indexOf("wa.me") > -1)
    ) {
...
function t1093__getBlockById(elementId) {
  var pairedRecord =
    document.getElementById("rec" + elementId) || document.getElementById(elementId);
  if (!pairedRecord) {
  ...
  var recordType =
    pairedRecord.getAttribute("data-parenttplid") || pairedRecord.getAttribute("data-record-type");
...
function t1093__showOnClick(e) {
  ...
  if (lastOpenedPopupHook) {
    ...
    var shouldClosePopup =
      lastOpenedPopup &&
      lastOpenedPopup.getAttribute("data-popup-close-on-open-another") === "true";
    if (shouldClosePopup) {
...
function t1093__getTimer(popup, action) {
  ...
  if ((isZeroTimer && action === "close") || !popup.classList.contains("t-popup-transition"))
    return 0;
...
function t1093__closePopup_isLastPopupWithLockedScroll() {
  var popups = t1093__getPopupObjValue("openPopUpList")
    ...
    .filter(Boolean);
  return popups.every((popup) => t1093__getBooleanPopupAttribute(popup, "disable-lock-scroll"));
}
